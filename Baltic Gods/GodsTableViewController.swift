...
class GodsTableViewController: UITableViewController {
    ...
    let balticGods = BalticGodData().getAllGods()
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        return balticGods.count
    }

    
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "balticGodsTableViewCell", for: indexPath)

        let god = balticGods[indexPath.row]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let indexPath = tableView.indexPathForSelectedRow{
            if let vc = segue.destination as? GodDetailViewController{
                vc.god = balticGods[indexPath.row]
            }
        }
